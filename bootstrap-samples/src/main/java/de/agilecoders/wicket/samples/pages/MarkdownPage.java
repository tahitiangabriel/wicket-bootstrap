...
public class MarkdownPage extends BasePage {
     ...
     */
    public MarkdownPage(final PageParameters parameters) {
        super(parameters);

        addForm("", false);
    }

    private void addForm(final String idSuffix, final boolean isEditable) {
        ...
        final MarkdownTextArea editor = new MarkdownTextArea("markdown" + idSuffix, model);
        final MarkdownConfig markdownConfig = editor.getConfig();
        markdownConfig.withHeight("550px").withWidth("col-lg-8").withResize(MarkdownConfig.Resize.both)
                .withIconLibrary(MarkdownConfig.IconLibrary.fa).withAutofocus(true);
